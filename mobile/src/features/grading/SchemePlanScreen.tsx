...
export interface SchemePlanProps {
  courseId: string
  phoneView?: "estructura" | "calendario"
  onPhoneViewChange?: (view: "estructura" | "calendario") => void
  onBack: () => void
}
...
function WeightBar({ total }: { total: number }) {
  ...
  const bar = ok ? "bg-emerald-500" : over ? "bg-red-500" : "bg-amber-400"
  return (
    <div className="space-y-1">
      ...
      <p
        className={`text-[11px] font-mono ${
          ok ? "text-emerald-700" : over ? "text-red-600" : "text-amber-700"
        }`}
      >
...
export function SchemePlanScreen({
  courseId,
  phoneView,
  onPhoneViewChange,
  onBack,
  ...
  const [phoneTabState, setPhoneTabState] =
    useState<"estructura" | "calendario">(phoneView ?? "estructura")
  const phoneTab = phoneView ?? phoneTabState
  const setPhoneTab = onPhoneViewChange ?? setPhoneTabState
  ...
  async function saveSegment() {
    ...
    if (weight > remaining + WEIGHT_SUM_TOLERANCE) {
      setActionError(
        `El peso no puede superar el ${formatWeight(remaining)}% restante.`,
      )
      return
    }
    setActionError("")
  ...
  async function addTemplate(
    ...
    if (!scheme) return
    const existing = segments.filter(
      (s) => s.subject_component === component.id,
    )
    if (
      existing.some(
        (s) => s.name.trim().toLowerCase() === template.name.toLowerCase(),
      )
    ) {
      return
    }
  ...
  async function removeSegment(segment: ComponentSegment) {
    if (
      !window.confirm(
        `¿Eliminar el segmento “${segment.name}” y sus actividades?`,
      )
    )
      return
    setActionError("")
    ...
    try {
      if (activityForm.editing) {
        ...
        const existing = activities.filter(
          (a) => a.segment === activityForm.segmentId,
        )
  ...
  if (components.length === 0) {
    return (
      <p className="text-sm text-slate-400 text-center py-8 px-4">
        Esta asignatura no tiene componentes en el catálogo. Pide a coordinación
        que los configure.
      </p>
    )
  }
        ...
        .map((component) => {
          ...
          return (
            <Card key={component.id} className="overflow-hidden">
              ...
              {open && (
                <div className="px-4 pb-4 space-y-3 border-t border-slate-100 max-h-[50dvh] overflow-y-auto overscroll-contain md:max-h-none md:overflow-visible">
    ...
    <div className="p-4 space-y-3">
      <div className="flex items-center justify-between">
        <button
          type="button"
          onClick={onPrev}
          className="w-9 h-9 rounded-full hover:bg-white text-slate-600"
        >
          ‹
        </button>
        <p className="text-sm font-semibold text-slate-800">
      ...
      {selectedDate && (
        <Card className="p-3 space-y-2">
          ...
          {activityForm && activityForm.date === selectedDate ? (
            ...
            <p className="text-[11px] text-slate-400">
              Crea un segmento en la estructura para poder añadir actividades.
            </p>
          )}
        </Card>
      )}
