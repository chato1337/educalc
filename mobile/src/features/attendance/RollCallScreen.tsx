...
export interface RollCallProps {
  ...
  date?: string
  onOriginChange?: (origin: RollCallOrigin) => void
...
function initialMark(student: RollCallRosterStudent): RollCallMark {
  return {
    status: isRollCallStatus(student.status) ? student.status : null,
    notes: student.notes ?? "",
  }
}
...
function buildEntries(
  ...
  const entries: RollCallEntryRequest[] = []
  for (const student of students) {
    const mark = marks[student.student] ?? initialMark(student)
...
export function RollCallScreen({
  ...
  const [dateState, setDateState] = useState(dateProp ?? todayIso)
  const [originState, setOriginState] = useState<RollCallOrigin>(origin)
  const date = dateProp ?? dateState
  const currentOrigin = onOriginChange ? origin : originState
  const setDate = onDateChange ?? setDateState
  const setCurrentOrigin = onOriginChange ?? setOriginState
  ...
  const assignmentId = currentOrigin === "subject" ? courseId : undefined
  ...
  const setFallbackPeriodId = useRollCallDraftStore(
    (s) => s.setFallbackPeriodId,
  )
  ...
  const marks = useMemo(() => {
    ...
    for (const student of students) {
      merged[student.student] =
        draft?.marks[student.student] ?? initialMark(student)
    }
    return merged
  }, [students, draft])
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter(
      (s) =>
        s.full_name.toLowerCase().includes(q) || s.document_number.includes(q),
    )
  }, [students, search])
  ...
  const unmarkedCount = students.filter((s) => !marks[s.student]?.status).length
  ...
  const updateStatus = (studentId: string, status: RollCallStatus) => {
    ...
    setMark(draftKey, studentId, {
      status: nextStatus,
      notes:
        nextStatus === "PRESENT" || nextStatus == null
          ? ""
          : (prev?.notes ?? ""),
    })
    if (nextStatus === "EXCUSED") setShowExcuseFor(studentId)
  ...
  const handleSave = async () => {
    ...
    if (missingPeriod && !fallbackPeriodId) {
      ...
      setFormError(
        "Esta fecha no cae en un periodo. Elige uno antes de guardar.",
      )
      return
    }
    if (unmarkedCount > 0) {
      setWriteFailed(false)
      setFormError("Marca a todos los estudiantes antes de guardar.")
      return
    }
      ...
      <div className="flex flex-col h-full bg-white items-center justify-center gap-4 p-8">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500 mt-1">
            {savedResult.present_count} presentes · {savedResult.excused_count}{" "}
            CE · {savedResult.unexcused_count} SE
          </p>
          <p className="text-xs text-slate-400 mt-2">
            {savedResult.created} nuevas · {savedResult.updated} actualizadas ·{" "}
            {savedResult.students_processed} estudiantes
          </p>
    ...
    <div className="flex flex-col h-full bg-[#F1F5F9]">
      ...
      {otherSourcesCount > 0 && (
        <div className="px-4 py-2 bg-blue-50 border-b border-blue-100 flex items-center gap-2 text-xs text-blue-700">
          ...
          Ya hay otro llamado hoy. El estado consolidado aplica por falta más
          grave.
        </div>
      )}
        ...
        {rosterQuery.isError && (
          <div className="px-2 py-8 text-center">
            <p className="text-sm text-slate-600">
              {rosterErrorMessage(rosterQuery.error)}
            </p>
            <button
              type="button"
        ...
        {rosterQuery.isSuccess && students.length > 0 && (
          <div className="grid grid-cols-1 min-[560px]:grid-cols-2 gap-1.5">
            {filtered.map((s) => {
              const mark = marks[s.student]
              const showNotes =
                showExcuseFor === s.student || mark?.status === "EXCUSED"
              return (
                <div key={s.student}>
