...
| **4 — Personas** | Hecho (CRUD) | `src/features/people/`: `TeachersPage`, `ParentsPage`, `UsersPage` (perfil RBAC: usuario Django pk, rol, institución, docente/acudiente opcional), `StudentGuardiansPage`, `GradeDirectorsPage` (filtros año/sede/grupo/docente; columna sede; formulario con docente, año y grupo del año). Estudiantes: `StudentFormPage` en `/students/new` y `/students/:id/edit`; botones en listado y ficha. Rutas quitadas de `resourceConfig.ts`. |
...
| **7 — Bulk load y pulido** | Hecho (MVP) | `BulkLoadHubPage` en `/bulk-load` (selector de tipo, multipart a cada `*/bulk-load/`, JSON de estadísticas). `/students/bulk-load` redirige a `/bulk-load`. Plantillas `docs/bulk_load_*.csv`. |
